namespace Admin.Controllers
			{
			  ...
			  {
					return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_alloweddevices?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
			  ...
			  public virtual async Task<IActionResult> EditPlatform(string usersid)
			  {
                    ...
                    if(HttpContext.Session.GetString("Timemajestyrole_JSON") != null){
                            ...
                            try{
                                ...
                                var jsonObjusers = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_users?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
                                if(jsonObjusers.Length > 2)
                                {
                                    ...
                                    HttpContext.Session.SetSession("TimemajestyusersModel", model);
                                    return View(model);
                                }
                                else
                                {
...
model.profilepicture = uploadFileName;
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<usersModel> ("TimemajestyusersModel");
...
model.profilepicture = modelCopy.profilepicture;
                                        ...
                                        if(model.profilepicture !=  null && model.profilepicture!="")
                                                HttpContext.Session.SetString("Timemajestyprofilepicture", model.profilepicture.ToString());
										strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/users/Update_Profile", model);
									}
							}
							else
...
public virtual async Task<IActionResult> Suspend_Profile(string usersid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/Suspend_Profile?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
...
Console.WriteLine(ex);
		...
		[HttpGet()]
			public virtual async Task<string> get_roles()
			{
				 ...
				 return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/get_roles?roles="+HttpContext.Session.GetString("Timemajestyuserrole"));
			...
			public virtual async Task<IActionResult> Login(userloginModel model)
			{
				...
				try
				{
						...
						if (dt.Rows.Count >0)
						{
							HttpContext.Session.SetString("Timemajestyusername", dt.Rows[0]["username"].ToString());
							HttpContext.Session.SetString("TimemajestyloginUserID", dt.Rows[0]["usersid"].ToString());
							
							HttpContext.Session.SetString("Timemajestytoken", dt.Rows[0]["token"].ToString());
							HttpContext.Session.SetString("Timemajestyuserrole", dt.Rows[0]["userrole"].ToString());
if(dt.Rows[0]["profilepicture"].ToString()!=null && dt.Rows[0]["profilepicture"].ToString()!="")
HttpContext.Session.SetString("Timemajestyprofilepicture", dt.Rows[0]["profilepicture"].ToString());
else
HttpContext.Session.SetString("Timemajestyprofilepicture", baseUrl+"/Icon_set/User.png");
							...
							message ="Login Success";
                                                                            ...
                                                                            HttpContext.Session.SetString("Timemajestychoosedtenantname", "");

																			HttpContext.Session.SetString("Timemajestychoosedtenantid", dt.Rows[0]["tenantid"].ToString());
							...
							if (response_roles.IsSuccessStatusCode)
							{
                                ...
                                if(dt_roles.Rows.Count ==0){
                                        ViewData["message"]="You are not authorized to access Timemajesty , Please contact administrator";
                                        return View(model); 
                                }
            
								HttpContext.Session.SetSession("Timemajestyroles", dt_roles);
								HttpContext.Session.SetString("Timemajestyrole_JSON", role_JSON);
							...
							if (response_menu.IsSuccessStatusCode)
							{
								...
								HttpContext.Session.SetSession("Timemajestymenu", dt_menu);
								HttpContext.Session.SetString("Timemajestymenu_JSON", menu_JSON);
				...
				if(message=="Login Success"){
					...
					return RedirectToAction("UpdateProfile","users", new {@usersid=HttpContext.Session.GetString("TimemajestyloginUserID")});
        ...
        public virtual async Task<String> appregister([FromBody] userloginModel model)
        {
            ...
            try
            {
                ...
                if (message == "")
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
							...
							if (response_roles.IsSuccessStatusCode)
							{
								...
								DataTable dt_roles = await response_roles.Content.ReadAsAsync<DataTable>();
								if(dt_roles.Rows.Count ==0){
                                        message="You are not authorized to access Timemajesty , Please contact administrator";
                    ...
                    {
                        message = "Timemajesty's API Response Failed";
                    }
                }
                else
                {
...
;
			...
			public virtual IActionResult IndexPlatform()
					{
						 HttpContext.Session.SetString("Timemajestychoosedtenantid","");
						return View();
					}	
			[HttpGet()]
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/List_of_User_Profiles?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID")
);
			}
