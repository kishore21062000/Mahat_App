			...
			public virtual async Task<IActionResult> Create_tenant(tenantModel model, IFormCollection collection)
			{
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
				 TempData["message"] = "Success";
						...
						HttpContext.Session.SetString("Timemajestychoosedtenantid",model.tenantid.ToString());
			  ...
			  {
					var jsonObjtenant = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/getById_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
					if(jsonObjtenant.Length > 2)
					{
						...
						HttpContext.Session.SetSession("TimemajestytenantModel", model);
						return View(model);
					}
					else
					{
					...
					return RedirectToAction(redirectTo);
							...
							if (ModelState.IsValid)
							{
									...
									else
									{
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<tenantModel> ("TimemajestytenantModel");
...
public virtual async Task<IActionResult> Remove_tenant(string tenantid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/Remove_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
...
Console.WriteLine(ex);
			...
			public virtual async Task<string> get_ViewList_tenant()
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/ViewList_tenant?loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID")
);
			}
