namespace Client.Controllers
			{
				...
				//This code generated from Deliveries Powered by Mahat, Source Machine : 15.206.208.9 , Build Number : Build 14092021 #2021-09-014(Updated on 29102021 12:49) on 07/20/2023 16:16:43
			...
			public virtual async Task<IActionResult> Create_MailLog(MailLogsModel model, IFormCollection collection)
			{
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
			  ...
			  {
					var jsonObjMailLogs = await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/getById_MailLogs?MailLogsid="+MailLogsid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
					if(jsonObjMailLogs.Length > 2)
					{
						...
						HttpContext.Session.SetSession("TimemajestyMailLogsModel", model);
						return View(model);
					}
					else
					{
					...
					return RedirectToAction(redirectTo);
							...
							if (ModelState.IsValid)
							{
									...
									else
									{
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<MailLogsModel> ("TimemajestyMailLogsModel");
...
public virtual async Task<IActionResult> Remove_MailLog(string MailLogsid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/Remove_MailLog?MailLogsid="+MailLogsid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
...
Console.WriteLine(ex);
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/MailLogs_List?entityname="+entityname+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID")
);
			}
