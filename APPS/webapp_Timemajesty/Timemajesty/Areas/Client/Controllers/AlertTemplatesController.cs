namespace Client.Controllers
			...
			public virtual async Task<IActionResult> Create_Alert_Templates(AlertTemplatesModel model, IFormCollection collection)
			{
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
			  ...
			  {
					var jsonObjAlertTemplates = await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/getById_AlertTemplates?AlertTemplatesid="+AlertTemplatesid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
					if(jsonObjAlertTemplates.Length > 2)
					{
						...
						HttpContext.Session.SetSession("TimemajestyAlertTemplatesModel", model);
						return View(model);
					}
					else
					{
					...
					return RedirectToAction(redirectTo);
							...
							if (ModelState.IsValid)
							{
									...
									else
									{
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<AlertTemplatesModel> ("TimemajestyAlertTemplatesModel");
...
public virtual async Task<IActionResult> Remove_Alert_Templates(string AlertTemplatesid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/Remove_Alert_Templates?AlertTemplatesid="+AlertTemplatesid+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID"));
...
,string entityaction
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/Alert_Templates_List?entityname="+entityname+"&entityaction="+entityaction+"&loginUserID="+HttpContext.Session.GetString("TimemajestyloginUserID")
);
			}
