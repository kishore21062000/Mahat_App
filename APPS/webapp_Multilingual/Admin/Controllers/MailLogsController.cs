namespace Admin.Controllers
			...
			public virtual async Task<IActionResult> Create_MailLog(MailLogsModel model, IFormCollection collection)
			{
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
			  ...
			  {
					var jsonObjMailLogs = await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/getById_MailLogs?MailLogsid="+MailLogsid+"&loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID"));
					if(jsonObjMailLogs.Length > 2)
					{
						...
						HttpContext.Session.SetSession("MultilingualMailLogsModel", model);
						return View(model);
					}
					else
					{
					...
					return RedirectToAction(redirectTo);
							...
							if (ModelState.IsValid)
							{
									...
									else
									{
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<MailLogsModel> ("MultilingualMailLogsModel");
...
public virtual async Task<IActionResult> Remove_MailLog(string MailLogsid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/Remove_MailLog?MailLogsid="+MailLogsid+"&loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID"));
...
Console.WriteLine(ex);
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/MailLogs/MailLogs_List?entityname="+entityname+"&loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID")
);
			}
