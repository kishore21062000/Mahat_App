namespace Admin.Controllers
			{
				...
				//This code generated from Deliveries Powered by Mahat, Source Machine : 15.206.208.9 , Build Number : Build 14092021 #2021-09-014(Updated on 29102021 12:49) on 07/08/2023 10:19:56
...
model.organizationlogo += "|" + uploadFileName +"|";
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
				 TempData["message"] = "Success";
						...
						HttpContext.Session.SetString("Multilingualchoosedtenantid",model.tenantid.ToString());
						TempData["notify"]="Please create user for the tenant";
							 
						if(HttpContext.Session.GetString("Multilingualrole_JSON") != null){
					DataTable Multilingualrole_JSON =HttpContext.Session.GetSession<DataTable>("Multilingualroles");
						 DataView dv = new DataView(Multilingualrole_JSON);
						 dv.RowFilter = "controllername='users' AND viewname='create'";
						...
						return RedirectToAction(redirectTo,"users");
			 }else{
			  ...
			  {
					var jsonObjtenant = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/getById_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID"));
					if(jsonObjtenant.Length > 2)
					{
						...
						HttpContext.Session.SetSession("MultilingualtenantModel", model);
						return View(model);
					}
					else
					{
...
model.organizationlogo += "|" + uploadFileName +"|";
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<tenantModel> ("MultilingualtenantModel");
...
public virtual async Task<IActionResult> Remove_tenant(string tenantid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/Remove_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID"));
...
Console.WriteLine(ex);
			...
			public virtual async Task<string> get_ViewList_tenant()
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/ViewList_tenant?loginUserID="+HttpContext.Session.GetString("MultilingualloginUserID")
);
			}
