...
ExistFile="$ROOTDIRECTORY"/Gmailconsole.Models/Gmailconsole.Models.csproj
if [ -f "$ExistFile" ]; then
	echo "Model File Found"
else
	echo cma2020# | sudo -S  dotnet new classlib -n Gmailconsole.Models --framework netcoreapp3.1 --force
	cd "$ROOTDIRECTORY"/Gmailconsole.Models
...
ExistFile="$ROOTDIRECTORY"/Gmailconsole.DAL/Gmailconsole.DAL.csproj
if [ -f "$ExistFile" ]; then
	echo "DAL File Found"
else
    cd "$ROOTDIRECTORY"
	echo cma2020# | sudo -S  dotnet new classlib -n Gmailconsole.DAL --framework netcoreapp3.1 --force
	echo cma2020# | sudo -S  dotnet add Gmailconsole.DAL/Gmailconsole.DAL.csproj reference Gmailconsole.Models/Gmailconsole.Models.csproj
	cd Gmailconsole.DAL
...
cd "$ROOTDIRECTORY"/Gmailconsole.DAL
echo "The current directory is : Expecting DAL folder"
pwd
echo cma2020# | sudo -S  dotnet build

ExistFile="$ROOTDIRECTORY"/Gmailconsole/Gmailconsole.csproj
if [ -f "$ExistFile" ]; then
	echo "Gmailconsole Web found"
else
	cd "$ROOTDIRECTORY"
	echo cma2020# | sudo -S  dotnet new web -n Gmailconsole --framework netcoreapp3.1 --force
	echo "This is after Web Gmailconsole create"
	echo cma2020# | sudo -S  cp -f "$ROOTDIRECTORY"/PreRequisites/*.* "$ROOTDIRECTORY"/Gmailconsole/
	echo cma2020# | sudo -S  dotnet add Gmailconsole/Gmailconsole.csproj reference Gmailconsole.Models/Gmailconsole.Models.csproj
	echo cma2020# | sudo -S  dotnet add Gmailconsole/Gmailconsole.csproj reference Gmailconsole.DAL/Gmailconsole.DAL.csproj
...
echo "Into the build section for Gmailconsole"
cd "$ROOTDIRECTORY"/Gmailconsole
echo cma2020# | sudo -S  dotnet build
echo cma2020# | sudo -S  dotnet publish -o "$ROOTDIRECTORY"/Publish/Gmailconsole

echo "Setting up the Publish Evnrionment"
cd /home/ubuntu/Automaton/AutomatonClient/wwwroot/PublishedFiles
echo cma2020# | sudo -S  chown -R ubuntu Gmailconsole
cd "$ROOTDIRECTORY"
echo cma2020# | sudo -S  rm -f /etc/nginx/sites-enabled/sandbox83
echo cma2020# | sudo -S  rm -f /etc/supervisor/conf.d/Gmailconsole*.conf                
...
echo cma2020# | sudo -S  supervisorctl restart GmailconsoleGmailconsole
echo cma2020# | sudo -S  service nginx reload
curl -v --header "Connection: keep-alive" "http://localhost:5011/ContactUs/sentPublishedNotification?projectid=df45d1a5-472d-419d-a508-bf4b52769a10"
