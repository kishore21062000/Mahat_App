namespace Admin.Controllers
			{
				...
				//This code generated from Deliveries Powered by Mahat, Source Machine : 15.206.208.9 , Build Number : Build 14092021 #2021-09-014(Updated on 29102021 12:49) on 09/12/2023 09:49:23
			  ...
			  {
					return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_alloweddevices?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("GmailconsoleloginUserID"));
...
model.profilepicture = uploadFileName;
			 ...
			 {
                _logger.LogError("Error occured in - users / CreatePlatform , Error Message : " + ex.Message);
			  ...
			  public virtual async Task<IActionResult> EditPlatform(string usersid)
			  {
                    ...
                    if(HttpContext.Session.GetString("Gmailconsolerole_JSON") != null){
                            ...
                            try{
                                ...
                                var jsonObjusers = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_users?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("GmailconsoleloginUserID"));
                                if(jsonObjusers.Length > 2)
                                {
                                    ...
                                    HttpContext.Session.SetSession("GmailconsoleusersModel", model);
                                    return View(model);
                                }
                                else
                                {
                                ...
                                _logger.LogError("Error occured while fetching data - users / EditPlatform , Error Message : " + ex.Message);
                                Console.WriteLine(ex);
...
model.profilepicture = uploadFileName;
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<usersModel> ("GmailconsoleusersModel");
...
model.profilepicture = modelCopy.profilepicture;
					...
					{
                        _logger.LogError("Error occured in - users / EditPlatform , Error Message : " + ex.Message);
Console.WriteLine(ex);
						strReturnMessage = ex.Message;
					}
					ViewData["message"] = strReturnMessage;
...
model.profilepicture = modelCopy.profilepicture;
                                        ...
                                        if(model.profilepicture !=  null && model.profilepicture!="")
                                                HttpContext.Session.SetString("Gmailconsoleprofilepicture", model.profilepicture.ToString());
										strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/users/Update_Profile", model);
									}
							}
							else
...
public virtual async Task<IActionResult> Suspend_Profile(string usersid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/Suspend_Profile?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("GmailconsoleloginUserID"));
...
public virtual async Task<IActionResult> DeletePlatform(string usersid)
			{
				...
				{
                _logger.LogError("Error occured in - users / DeletePlatform , Error Message : " + ex.Message);
Console.WriteLine(ex);
				message = ex.Message;
				}
		...
		[HttpGet()]
			public virtual async Task<string> get_roles()
			{
				 ...
				 return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/get_roles?roles="+HttpContext.Session.GetString("Gmailconsoleuserrole"));
			...
			public virtual async Task<IActionResult> Login(userloginModel model)
			{
				...
				try
				{
						...
						if (dt.Rows.Count >0)
						{
							HttpContext.Session.SetString("Gmailconsoleusername", dt.Rows[0]["username"].ToString());
							HttpContext.Session.SetString("GmailconsoleloginUserID", dt.Rows[0]["usersid"].ToString());
							
							HttpContext.Session.SetString("Gmailconsoletoken", dt.Rows[0]["token"].ToString());
							HttpContext.Session.SetString("Gmailconsoleuserrole", dt.Rows[0]["userrole"].ToString());
if(dt.Rows[0]["profilepicture"].ToString()!=null && dt.Rows[0]["profilepicture"].ToString()!="")
HttpContext.Session.SetString("Gmailconsoleprofilepicture", dt.Rows[0]["profilepicture"].ToString());
else
HttpContext.Session.SetString("Gmailconsoleprofilepicture", baseUrl+"/Icon_set/User.png");
							...
							message ="Login Success";
                                                                            ...
                                                                            HttpContext.Session.SetString("Gmailconsolechoosedtenantname", "");

																			HttpContext.Session.SetString("Gmailconsolechoosedtenantid", dt.Rows[0]["tenantid"].ToString());
							...
							if (response_roles.IsSuccessStatusCode)
							{
                                ...
                                if(dt_roles.Rows.Count ==0){
                                        ViewData["message"]="You are not authorized to access Gmailconsole , Please contact administrator";
                                        return View(model); 
                                }
            
								HttpContext.Session.SetSession("Gmailconsoleroles", dt_roles);
								HttpContext.Session.SetString("Gmailconsolerole_JSON", role_JSON);
							...
							if (response_menu.IsSuccessStatusCode)
							{
								...
								HttpContext.Session.SetSession("Gmailconsolemenu", dt_menu);
								HttpContext.Session.SetString("Gmailconsolemenu_JSON", menu_JSON);
				...
				if(message=="Login Success"){
					...
					return RedirectToAction("UpdateProfile","users", new {@usersid=HttpContext.Session.GetString("GmailconsoleloginUserID")});
        ...
        public virtual async Task<String> appregister([FromBody] userloginModel model)
        {
            ...
            try
            {
                ...
                if (message == "")
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
							...
							if (response_roles.IsSuccessStatusCode)
							{
								...
								DataTable dt_roles = await response_roles.Content.ReadAsAsync<DataTable>();
								if(dt_roles.Rows.Count ==0){
                                        message="You are not authorized to access Gmailconsole , Please contact administrator";
                    ...
                    {
                        message = "Gmailconsole's API Response Failed";
                    }
                }
                else
                {
...
;
			...
			public virtual IActionResult IndexPlatform()
					{
						 HttpContext.Session.SetString("Gmailconsolechoosedtenantid","");
						return View();
					}	
			[HttpGet()]
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/List_of_User_Profiles?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("GmailconsoleloginUserID")
);
			}
