...
model.organizationlogo += "|" + uploadFileName +"|";
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
				 TempData["message"] = "Success";
						...
						HttpContext.Session.SetString("AIDatachoosedtenantid",model.tenantid.ToString());
			  ...
			  {
					var jsonObjtenant = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/getById_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID"));
					if(jsonObjtenant.Length > 2)
					{
						...
						HttpContext.Session.SetSession("AIDatatenantModel", model);
						return View(model);
					}
					else
					{
...
model.organizationlogo += "|" + uploadFileName +"|";
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<tenantModel> ("AIDatatenantModel");
...
public virtual async Task<IActionResult> Remove_tenant(string tenantid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/Remove_tenant?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID"));
...
Console.WriteLine(ex);
			...
			public virtual async Task<string> get_ViewList_tenant()
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/tenant/ViewList_tenant?loginUserID="+HttpContext.Session.GetString("AIDataloginUserID")
);
			}
