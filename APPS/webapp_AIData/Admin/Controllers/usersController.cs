namespace Admin.Controllers
			{
			  ...
			  {
					return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_alloweddevices?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID"));
			  ...
			  public virtual async Task<IActionResult> EditPlatform(string usersid)
			  {
                    ...
                    if(HttpContext.Session.GetString("AIDatarole_JSON") != null){
                            ...
                            try{
                                ...
                                var jsonObjusers = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_users?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID"));
                                if(jsonObjusers.Length > 2)
                                {
                                    ...
                                    HttpContext.Session.SetSession("AIDatausersModel", model);
                                    return View(model);
                                }
                                else
                                {
...
model.profilepicture = uploadFileName;
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<usersModel> ("AIDatausersModel");
...
model.profilepicture = modelCopy.profilepicture;
                                        ...
                                        if(model.profilepicture !=  null && model.profilepicture!="")
                                                HttpContext.Session.SetString("AIDataprofilepicture", model.profilepicture.ToString());
										strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/users/Update_Profile", model);
									}
							}
							else
...
public virtual async Task<IActionResult> Suspend_Profile(string usersid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/Suspend_Profile?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID"));
...
Console.WriteLine(ex);
		...
		[HttpGet()]
			public virtual async Task<string> get_roles()
			{
				 ...
				 return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/get_roles?roles="+HttpContext.Session.GetString("AIDatauserrole"));
			...
			public virtual async Task<IActionResult> Login(userloginModel model)
			{
				...
				try
				{
						...
						if (dt.Rows.Count >0)
						{
							HttpContext.Session.SetString("AIDatausername", dt.Rows[0]["username"].ToString());
							HttpContext.Session.SetString("AIDataloginUserID", dt.Rows[0]["usersid"].ToString());
							
							HttpContext.Session.SetString("AIDatatoken", dt.Rows[0]["token"].ToString());
							HttpContext.Session.SetString("AIDatauserrole", dt.Rows[0]["userrole"].ToString());
if(dt.Rows[0]["profilepicture"].ToString()!=null && dt.Rows[0]["profilepicture"].ToString()!="")
HttpContext.Session.SetString("AIDataprofilepicture", dt.Rows[0]["profilepicture"].ToString());
else
HttpContext.Session.SetString("AIDataprofilepicture", baseUrl+"/Icon_set/User.png");
							...
							message ="Login Success";
                                                                            ...
                                                                            HttpContext.Session.SetString("AIDatachoosedtenantname", "");

																			HttpContext.Session.SetString("AIDatachoosedtenantid", dt.Rows[0]["tenantid"].ToString());
							...
							if (response_roles.IsSuccessStatusCode)
							{
                                ...
                                if(dt_roles.Rows.Count ==0){
                                        ViewData["message"]="You are not authorized to access AIData , Please contact administrator";
                                        return View(model); 
                                }
            
								HttpContext.Session.SetSession("AIDataroles", dt_roles);
								HttpContext.Session.SetString("AIDatarole_JSON", role_JSON);
							...
							if (response_menu.IsSuccessStatusCode)
							{
								...
								HttpContext.Session.SetSession("AIDatamenu", dt_menu);
								HttpContext.Session.SetString("AIDatamenu_JSON", menu_JSON);
				...
				if(message=="Login Success"){
					...
					return RedirectToAction("UpdateProfile","users", new {@usersid=HttpContext.Session.GetString("AIDataloginUserID")});
        ...
        public virtual async Task<String> appregister([FromBody] userloginModel model)
        {
            ...
            try
            {
                ...
                if (message == "")
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
							...
							if (response_roles.IsSuccessStatusCode)
							{
								...
								DataTable dt_roles = await response_roles.Content.ReadAsAsync<DataTable>();
								if(dt_roles.Rows.Count ==0){
                                        message="You are not authorized to access AIData , Please contact administrator";
                    ...
                    {
                        message = "AIData's API Response Failed";
                    }
                }
                else
                {
...
;
			...
			public virtual IActionResult IndexPlatform()
					{
						 HttpContext.Session.SetString("AIDatachoosedtenantid","");
						return View();
					}	
			[HttpGet()]
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/List_of_User_Profiles?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("AIDataloginUserID")
);
			}
