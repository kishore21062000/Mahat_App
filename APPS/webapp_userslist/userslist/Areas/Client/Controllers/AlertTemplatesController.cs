namespace Client.Controllers
			{
				...
				//This code generated from Deliveries Powered by Mahat, Source Machine : 15.206.208.9 , Build Number : Build 14092021 #2021-09-014(Updated on 29102021 12:49) on 09/07/2023 11:18:47
			...
			public virtual async Task<IActionResult> Create_Alert_Templates(AlertTemplatesModel model, IFormCollection collection)
			{
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
			  ...
			  {
					var jsonObjAlertTemplates = await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/getById_AlertTemplates?AlertTemplatesid="+AlertTemplatesid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID"));
					if(jsonObjAlertTemplates.Length > 2)
					{
						...
						HttpContext.Session.SetSession("userslistAlertTemplatesModel", model);
						return View(model);
					}
					else
					{
					...
					return RedirectToAction(redirectTo);
							...
							if (ModelState.IsValid)
							{
									...
									else
									{
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<AlertTemplatesModel> ("userslistAlertTemplatesModel");
...
public virtual async Task<IActionResult> Remove_Alert_Templates(string AlertTemplatesid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/Remove_Alert_Templates?AlertTemplatesid="+AlertTemplatesid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID"));
...
,string entityaction
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/AlertTemplates/Alert_Templates_List?entityname="+entityname+"&entityaction="+entityaction+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID")
);
			}
