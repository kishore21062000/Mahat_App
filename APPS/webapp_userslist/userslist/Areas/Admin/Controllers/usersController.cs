namespace Admin.Controllers
			{
				...
				//This code generated from Deliveries Powered by Mahat, Source Machine : 15.206.208.9 , Build Number : Build 14092021 #2021-09-014(Updated on 29102021 12:49) on 09/07/2023 11:18:39
			  ...
			  {
					return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_alloweddevices?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID"));
			  ...
			  public virtual async Task<IActionResult> EditPlatform(string usersid)
			  {
                    ...
                    if(HttpContext.Session.GetString("userslistrole_JSON") != null){
                            ...
                            try{
                                ...
                                var jsonObjusers = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/getById_users?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID"));
                                if(jsonObjusers.Length > 2)
                                {
                                    ...
                                    HttpContext.Session.SetSession("userslistusersModel", model);
                                    return View(model);
                                }
                                else
                                {
...
model.profilepicture = uploadFileName;
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<usersModel> ("userslistusersModel");
...
model.profilepicture = modelCopy.profilepicture;
                                        ...
                                        if(model.profilepicture !=  null && model.profilepicture!="")
                                                HttpContext.Session.SetString("userslistprofilepicture", model.profilepicture.ToString());
										strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/users/Update_Profile", model);
									}
							}
							else
...
public virtual async Task<IActionResult> Suspend_Profile(string usersid)
			{
				...
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/users/Suspend_Profile?usersid="+usersid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID"));
...
Console.WriteLine(ex);
		...
		[HttpGet()]
			public virtual async Task<string> get_roles()
			{
				 ...
				 return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/get_roles?roles="+HttpContext.Session.GetString("userslistuserrole"));
			...
			public virtual async Task<IActionResult> Login(userloginModel model)
			{
				...
				try
				{
						...
						if (dt.Rows.Count >0)
						{
							HttpContext.Session.SetString("userslistusername", dt.Rows[0]["username"].ToString());
							HttpContext.Session.SetString("userslistloginUserID", dt.Rows[0]["usersid"].ToString());
							
							HttpContext.Session.SetString("userslisttoken", dt.Rows[0]["token"].ToString());
							HttpContext.Session.SetString("userslistuserrole", dt.Rows[0]["userrole"].ToString());
if(dt.Rows[0]["profilepicture"].ToString()!=null && dt.Rows[0]["profilepicture"].ToString()!="")
HttpContext.Session.SetString("userslistprofilepicture", dt.Rows[0]["profilepicture"].ToString());
else
HttpContext.Session.SetString("userslistprofilepicture", baseUrl+"/Icon_set/User.png");
							...
							message ="Login Success";
                                                                            ...
                                                                            HttpContext.Session.SetString("userslistchoosedtenantname", "");

																			HttpContext.Session.SetString("userslistchoosedtenantid", dt.Rows[0]["tenantid"].ToString());
							...
							if (response_roles.IsSuccessStatusCode)
							{
                                ...
                                if(dt_roles.Rows.Count ==0){
                                        ViewData["message"]="You are not authorized to access userslist , Please contact administrator";
                                        return View(model); 
                                }
            
								HttpContext.Session.SetSession("userslistroles", dt_roles);
								HttpContext.Session.SetString("userslistrole_JSON", role_JSON);
							...
							if (response_menu.IsSuccessStatusCode)
							{
								...
								HttpContext.Session.SetSession("userslistmenu", dt_menu);
								HttpContext.Session.SetString("userslistmenu_JSON", menu_JSON);
				...
				if(message=="Login Success"){
					...
					return RedirectToAction("UpdateProfile","users", new {@usersid=HttpContext.Session.GetString("userslistloginUserID")});
        ...
        public virtual async Task<String> appregister([FromBody] userloginModel model)
        {
            ...
            try
            {
                ...
                if (message == "")
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (dt.Rows.Count > 0)
                        {
							...
							if (response_roles.IsSuccessStatusCode)
							{
								...
								DataTable dt_roles = await response_roles.Content.ReadAsAsync<DataTable>();
								if(dt_roles.Rows.Count ==0){
                                        message="You are not authorized to access userslist , Please contact administrator";
                    ...
                    {
                        message = "userslist's API Response Failed";
                    }
                }
                else
                {
...
;
			...
			public virtual IActionResult IndexPlatform()
					{
						 HttpContext.Session.SetString("userslistchoosedtenantid","");
						return View();
					}	
			[HttpGet()]
...
)
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/users/List_of_User_Profiles?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("userslistloginUserID")
);
			}
