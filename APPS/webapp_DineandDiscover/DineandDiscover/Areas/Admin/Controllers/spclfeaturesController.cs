namespace Admin.Controllers
			{
				...
				{	 
                    ...
                    private readonly ILogger<spclfeaturesController> _logger;
					...
					public spclfeaturesController(IConfiguration configuration,IHttpContextAccessor accessor,IOptions<ApiSettings> ApiSettings, IOptions<MailSettings> MailSettings, IWebHostEnvironment env, ILogger<spclfeaturesController> logger)
					{
                        _logger = logger;
						this.hostingEnv = env;
			  ...
			  [HttpPost()]
			public virtual async Task<IActionResult> Add_specialfeatures_avail(spclfeaturesModel model, IFormCollection collection)
			{
				string strReturnMessage = "";
				string redirectTo="";
				try
				{
					ModelState.Remove("spclfeaturesid");
...
ModelState.Remove("videoofoverallrestaurant");
					...
					DataTable DineandDiscoverrole_JSON =HttpContext.Session.GetSession<DataTable>("DineandDiscoverroles");
						 ...
						 dv.RowFilter = "controllername='spclfeatures' AND viewname='list'";
...
string uploadFileName = System.Text.RegularExpressions.Regex.Replace(filename.Split('.').First(), @"[^0-9a-zA-Z_.]+", "").Replace(" ", String.Empty)+"_"+"spclfeatures_" +rnd.Next(1, 10000).ToString() + DateTime.Now.ToString("ddMMyyHHmmss")+ fileExtention;
if (fileExtention != ".")
{
...
model.videoofoverallrestaurant += "|" + uploadFileName +"|";
								 ...
								 strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/spclfeatures/Add_specialfeatures_avail", model);
							 }
					 }
					 else
					 {
			 ...
			 {
                _logger.LogError("Error occured in - spclfeatures / Add_specialfeatures_avail , Error Message : " + ex.Message);
			 ...
			 if(strReturnMessage.Replace("\"", "").Contains("201.1")){
			  ...
			  public virtual async Task<IActionResult> Update_specialfeatures_avail(string spclfeaturesid)
			  {
					var jsonObjspclfeatures = await ApiClient.Get_ApiValues(getHttpClient(), "api/spclfeatures/getById_spclfeatures?spclfeaturesid="+spclfeaturesid+"&loginUserID="+HttpContext.Session.GetString("DineandDiscoverloginUserID"));
					if(jsonObjspclfeatures.Length > 2)
					{
						jsonObjspclfeatures = jsonObjspclfeatures.Substring(1, jsonObjspclfeatures.Length - 2);
						var model = JsonConvert.DeserializeObject<spclfeaturesModel>(jsonObjspclfeatures);
						HttpContext.Session.SetSession("DineandDiscoverspclfeaturesModel", model);
						return View(model);
					}
					else
					{
			  ...
			  [HttpPost()]
				public virtual async Task<IActionResult> Update_specialfeatures_avail(spclfeaturesModel model, IFormCollection collection)
				{
					string strReturnMessage = "";
					string redirectTo="";
					try
...
model.videoofoverallrestaurant += "|" + uploadFileName +"|";
                                        ...
                                        var modelCopy = HttpContext.Session.GetSession<spclfeaturesModel> ("DineandDiscoverspclfeaturesModel");
...
model.videoofoverallrestaurant = modelCopy.videoofoverallrestaurant;
										...
										strReturnMessage = await ApiClient.Post_ApiValuesGetString(getHttpClient(),"api/spclfeatures/Update_specialfeatures_avail", model);
									}
							}
							else
							{
					...
					{
                        _logger.LogError("Error occured in - spclfeatures / Update_specialfeatures_avail , Error Message : " + ex.Message);
Console.WriteLine(ex);
						strReturnMessage = ex.Message;
					}
					ViewData["message"] = strReturnMessage;
...
public virtual async Task<IActionResult> Remove_specialfeatures_avail(string spclfeaturesid)
			{
				string message = "";
				try
				{
						message = await ApiClient.Get_ApiValues(getHttpClient(), "api/spclfeatures/Remove_specialfeatures_avail?spclfeaturesid="+spclfeaturesid+"&loginUserID="+HttpContext.Session.GetString("DineandDiscoverloginUserID"));
				...
				{
                    _logger.LogError("Error occured in - spclfeatures / Remove_specialfeatures_avail , Error Message : " + ex.Message);
Console.WriteLine(ex);
			...
			public virtual async Task<string> get_specialfeatures_avail_List()
			{
				...
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/spclfeatures/specialfeatures_avail_List?loginUserID="+HttpContext.Session.GetString("DineandDiscoverloginUserID")
);
			}
			 [HttpGet()]
			  ...
			  public virtual async Task<string> getById_sp_all_spclfeatures(string spclfeaturesid)
			  {
					return await ApiClient.Get_ApiValues(getHttpClient(), "api/spclfeatures/getById_sp_all_spclfeatures?spclfeaturesid="+spclfeaturesid);
					 
			  }
[HttpGet()]
			public virtual async Task<string> lookup_spclfeatures_spclfeatures(String tenantid)
			{
				return await ApiClient.Get_ApiValues(getHttpClient(), "api/spclfeatures/lookup_spclfeatures_spclfeatures?tenantid="+tenantid+"&loginUserID="+HttpContext.Session.GetString("DineandDiscoverloginUserID"));
			}
